...
export const createTodo = async (req, res) => {
  ...
  try {
    ...
    if (!userId && !title) response(res, 403, { error: "Missing required params" })


    const todoTask = await todoModel.create({ userId, title });

    response(res, 200, { message: "Todo Created", todoTask })
